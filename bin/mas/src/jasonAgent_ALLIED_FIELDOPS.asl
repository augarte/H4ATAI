...
+!perform_look_action
	...
	if(Cap == 0){
		...
		if (CurrentTime - FirstTime > 300 & not sent){
			-+initial_time(CurrentTime);
			+sent;
			// Captain assignment
			?assigned_id(AssignedID);
			.my_team("fieldops_ALLIED",Fs);
			.concat("assign_captain(", AssignedID, ")", MyID);
			.send_msg_with_conversation_id(Fs, tell, MyID, "AC");
			if (Mode==5){.println("I sent: ", MyID, " to: ", Fs);}
		}
	}
	// Captain assigned
	else{
...
+around_me(X,Y,Z)[source(M)]
	...
	?role(R);
	if (R == 0){OfSx=-8;OfSz=4;} if (R == 1){OfSx=-8;OfSz=-4;}
